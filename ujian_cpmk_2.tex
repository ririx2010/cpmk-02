...
Gambar~\ref{fig:decision-boundary} menunjukkan \emph{decision boundary}
yang memetakan NK sebagai fungsi dari IPK dan Minat secara serentak.
Warna menunjukkan NK: \textcolor{red}{merah} (rendah), kuning (sedang),
\textcolor{green}{hijau} (tinggi). Untuk data target \#20: SKJK
$(3.65;\,1.50)$ jatuh di zona merah/kuning (NK$=50$, rule R7),
ITP $(3.40;\,2.60)$ di zona kuning/hijau (NK$=71.14$), dan RPLD
$(3.52;\,2.80)$ di zona hijau (NK$=75$). Area kanan bawah (IP tinggi,
Minat rendah) tetap rendah --- konsekuensi rule R7 yang memastikan
IP tinggi saja tidak cukup tanpa minat.

% --- Gambar Decision Boundary (full-width) ----------------------------
\begin{figure*}[t!]
\centering
...
% --- Tabel VII: Hasil verifikasi 20 data ------------------------------
\begin{table*}[t!]
...
\begin{enumerate}[nosep]
  ...
  \item Aplikasi web interaktif tersedia untuk demo publik di:\\
        \url{https://cpmk-02-ririx.streamlit.app/}
\end{enumerate}
...
\section{Source Code: \texttt{sim\_fuzzy\_spk.py}}
\label{app:source}

\lstinputlisting[firstline=1,lastline=160,title={sim\_fuzzy\_spk.py --- FIS Mamdani (bagian inti)}]{sim_fuzzy_spk_clean.py}
